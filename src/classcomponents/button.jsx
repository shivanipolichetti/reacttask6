...
class ButtonComponent extends Component{
    ...
    addCard=()=>{
        const newCard=[...this.state.cards,"newCard"]
        //console.log(newCard);
       this.setState(
        {
            cards:newCard
...
deleteCard=(index)=>{
    const newCard=this.state.cards.filter((eachCard,i)=>i!==index)
...
updateCard=(index)=>{
    const newCard=this.state.cards.map((eachCard,i)=>{
        if(i===index){
            return {
               ...eachCard,
           ...
           <button onClick={this.changeSubscribe}>{this.state.isSubscribe?this.state.text1:this.state.text2}</button>{
            ...
            {this.state.cards.map((card,index)=>(
                ...
                <div key={card.id} className="card-form">
                    <img src={card.image} alt={card.title} width={100}></img>
                    <h1>{card.title}</h1>
                    <h1>{card.category}</h1>
                    <h1>{card.price}</h1>
                    <h1>{card.rating}</h1>
            ...
            :
            <h1>Please Subscribe to Shop</h1>
           }
        </div>
